...
$allowed_files = $theme->get_files( 'php',3 );
...
*/
		//$file_desc = get_file_description( $absolute_filename );
		
		/*if(!preg_match('/danish/i', $filename)) {
			echo '';
		}else{
			echo '<li><a href="theme-editor.php?file='.urlencode($filename).'&amp;theme='.urlencode($stylesheet ).'">'.get_file_description($filename).' <i>(Danish)</i></a></li>';
			//$file_description = get_file_description( $absolute_filename );
		}*/
		
		if(!preg_match('/english/i', $filename)) {
			echo '';
		}else{
			echo '<li><a href="theme-editor.php?file='.urlencode($filename).'&amp;theme='.urlencode($stylesheet ).'">'.ucfirst(str_replace(array('_','lang.php'),' ',get_file_description($filename))).'</a></li>';
			//$file_description = get_file_description( $absolute_filename );
		}
